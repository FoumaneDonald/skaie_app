...
export class Profile implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.user.set(u);
    if (u) {
      this.form.patchValue({ name: u.name, phone: u.phone ?? '', date_of_birth: u.date_of_birth ?? '' });
    }
  }
  ...
  savePw(): void {
    ...
    this.successMsg.set(null);
    this.errorMsg.set(null);
    this.http.patch(`${environment.apiUrl}/customer/profile/password`, this.pwForm.value).subscribe({
      next: () => {
        this.pwForm.reset();
